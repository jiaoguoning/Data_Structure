...
// 算法2.23 
// 思路：
//       1. 每个多项式定义两个指针，分别挨着向后移动；
//       2. 移动过程中Pb中每一项分别和Pa进行比较，（多项式根据指数有序）
//             若相等合并;
//             若小于Pa中项不动，指针后移;
//             若大于将Pb中该项插入Pa； 
//       3. 以Pa多项式为主体，Pb多项式进行比较依次进行插入合并；  
//       4. 最终Pa成为结果，也就是 Pa = Pa + Pb。
